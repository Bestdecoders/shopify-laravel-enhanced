...
const Sidebar = ({
  ...
  useEffect(() => {
    ...
    const fetchToken = async () => {
      if (shopify && shopify.idToken) {
        try {
          const sessionToken = await shopify.idToken(); // Fetch Shopify session token
  ...
  const secondaryNavigation = [
    {
      url: updateUrlWithHostAndToken("/pricing"),
      label: "Pricing",
      icon: ChartVerticalIcon,
      key: "pricing", // Unique key
    },

    {
      url: updateUrlWithHostAndToken("/docs"),
      label: "Documentation",
      icon: BookIcon,
      key: "documentation", // Unique key
    },
    {
      url: updateUrlWithHostAndToken("/faq"),
      label: "FAQ",
    ...
    <div className="fixed left-10 top-20 bor">
      <Frame>
        <Navigation location="/">
          ...
          <Navigation.Section title="Connect" items={secondaryNavigation} />
        </Navigation>
      </Frame>
    </div>
  );
